    ...
    if (response.action === 'new') {
      ...
      if (template.type === 'full') {
        ...
        try {
          ...
          if (!indexRes.ok) throw new Error('Falha ao buscar registry index');
          const registry = await indexRes.json() as { items: any[] };
    ...
    for (const component of components) {
      ...
      try {
        ...
        if (!options.overwrite) {
          const existingFiles = compData.files.filter(f =>
            fs.existsSync(path.join(projectPath, f.target))
          );
